...
Genome.prototype.mutate = function() {
  ...
  pos = Math.floor(Math.random() * this.genes.length);
  pos2 = Math.floor(Math.random() * this.genes.length);

  //ensures that the two elements are different
  while (pos === pos2) {
    pos2 = Math.floor(Math.random() * this.genes.length);
  };

  temp = this.genes[pos];
  this.genes[pos] = this.genes[pos2];
